/*
 * export.cpp
 * Exporta los paquetes capturados (que pasaron el filtro activo) a CSV.
 */
...
bool export_csv(const sniffer_state_t *state, const char *filename)
{
    ...
    for (int i = 0; i < state->count && i < MAX_PACKETS; i++)
    {
        ...
        fprintf(fp, "%d,%s,%s,%s,%s,%d,%d,%d,%d,%d,%d,%s,%d,%d\n",
                rec->index, rec->time_str, rec->proto_str,
                rec->src_ip, rec->dst_ip, rec->src_port, rec->dst_port,
                rec->total_len, rec->ip_id, rec->ttl, rec->tos,
                flags, rec->icmp_type, rec->icmp_code);
    }
